...
**运行环境（已锁定）**：开发与验题均在 **Linux + Docker**；判题时传统题 **严格比较输出文件**，或 **SPJ/checker**（见 §13.3）。
...
启动：`docker compose up`（api、worker、postgres、minio、web）。**禁止**前端 import 阶段 Agent 或 LangGraph 图。
...
| **M5** | 爬虫 + Worker 可选(D-08) + CLI 命令齐全 | compose 一键起 |
